...
define-command -hidden snippets-add-snippet-impl -params 2.. %{ evaluate-commands %sh{
    ...
    if [ -z "${trigger##*/*}" ]; then
        ...
        eval "set -- $kak_opt_snippets_directories"
        while [ $# -gt 0 ]; do
            menu="$menu %{$1/$filetype} %{evaluate-commands %{ nop %sh{ [ -d '$1/$filetype' ] || mkdir -p '$1/$filetype' }; edit %{$1/$filetype/$trigger - $description}; hook -once -group snippets-watcher global BufWritePost .* snippets-directory-reload}}"
            shift
        done
