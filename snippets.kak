...
define-command -docstring "snippets-create-snippet <trigger> <description> [<filetype>]: Create new snippet for given filetype.
If filetype is ommited, current active filetype is used.
If no current filetype available, snippet is created in global scope.
...
snippets-add-snippet -params 0..3 %{ evaluate-commands %sh{
    if [ $# -ge 2 ]; then
        ...
        description=$2
        filetype=$3
        printf "%s\n" "snippets-create-snippet-impl %{$trigger} %{$description} %{$filetype}"
    else
        printf "snippets-create-snippet-prompt\n"
    fi
}}
...
define-command -hidden snippets-add-snippet-prompt %{ evaluate-commands %{
    prompt "Trigger: " %{
        ...
        prompt "Snippet Description: " %{
            snippets-create-snippet-impl %opt{new_trigger} %val{text} %opt{filetype}
        }
    }
}}
...
define-command -hidden snippets-add-snippet-impl -params 2.. %{ evaluate-commands %sh{
    ...
    snippet_dir="$kak_config/snippets/$filetype"
    ...
    if [ -z "${trigger##*/*}" ]; then
        ...
        snippet_dir="'$snippet_dir/$trigger - $description'"
        printf "edit %s\n" "$snippet_dir"
    fi
}}
